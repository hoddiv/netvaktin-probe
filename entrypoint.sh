...
log "Starting Netvaktin Probe (Bulletproof v2.2)..."
...
if [ -n "$ZBX_API_TOKEN" ]; then
    ...
    if [ -f "$PSK_FILE" ]; then
        ...
        python3 /usr/bin/register_probe.py || log "⚠️ Registration script failed but continuing..."
    else
        log "⚠️ PSK file missing at $PSK_FILE. Registration skipped."
    fi
fi

# 3. Agent Config - Hardened with Concurrency Limits
cat > "$ZABBIX_CONF" <<EOF
...
# Prevent PID exhaustion by capping concurrent scripts
Plugins.SystemRun.Capacity=12
EOF
